...
*   `/getid` — Returns the Chat ID of the current chat (useful for finding your `ADMIN_CHAT_ID` or the `GROUP_CHAT_ID`).

*(Note: Teachers can answer questions simply by replying to the bot's post directly in the classroom group chat—no special command is needed for this.)*
